...
exports.list = async (req, res) => {
    try {
        ...
        res.render('items/index', { items: itemResult.rows, categoryNames, page: { title: 'Item List - Inventory App' }, selectedCategory: selectedCategory || 'all' });
    } catch (err) {
        console.error(err);
...
exports.show = async (req, res) => {
    ...
    res.render('items/show', { item: result.rows[0], category_name: result.rows[0].category_name, page: { title: 'Item Details - Inventory App' } });
};

exports.newForm = async(req, res) => {
    try {
        const categories = (await db.query('SELECT * FROM categories ORDER BY name')).rows;
        res.render('items/new', { categories, page: { title: 'Add Item - Inventory App' } });
    } catch (err) {
        console.error(err);
...
exports.editForm = async (req, res) => {
    try {
        ...
        res.render('items/edit', { item, categories: cats, page: { title: 'Edit Item - Inventory App' } });
    } catch (err) {
        console.error(err);
